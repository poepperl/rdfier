...
class Benchmark:
        ...
        for _ in tqdm(range(MEAN_LOOPS+1)):
            for model_index, model_numb in enumerate(modellist):
                ...
                for index, query_numb in enumerate(querylist):
                    ...
                    print(f"Run query {query_numb} of model {model_numb}. #uncertain cells = {len(self.graph_generator.rdfdata.uncertainties)}. #uncertain statements = {sum(altlist) if len(altlist)>0 else 0}", end="\r")
    ...
    def _plot_results_increasing_params(self, X : range, results : list[list[list[float]]], querylist : list[int], modellist : list[int], increasing_alternatives : bool):
